...
const CONFIG_FILE = path.join(os.homedir(), ".localchat", "config.json");
...
function loadConfig(): LocalChatConfig {
  try {
...
function saveConfig(cfg: LocalChatConfig): void {
...
program
  ...
  .action((opts) => {
    ...
    if (opts.name) cfg.mdnsName = opts.name;
    if (opts.port) cfg.port = parseInt(opts.port, 10);
    if (opts.persist) cfg.persist = true;
...
program
  ...
  .action((flag = "on") => {
    ...
    if (enabled) console.log(`  DB: ${cfg.dataDir}/localchat.db`);
    console.log(`  Run "localchat start" to apply.\n`);
  });
...
program
  ...
  .action(() => {
    const cfg = loadConfig();
    const dbPath = path.join(cfg.dataDir, "localchat.db");
