...
exports.createChannelInNetwork = createChannelInNetwork;
exports.listChannelsInNetwork = listChannelsInNetwork;
exports.deleteChannelInNetwork = deleteChannelInNetwork;
exports.getAllUsersInNetwork = getAllUsersInNetwork;
exports.appendLogInNetwork = appendLogInNetwork;
exports.getLogRoomInNetwork = getLogRoomInNetwork;
exports.searchMessagesInNetwork = searchMessagesInNetwork;
exports.connectionCountForUserInNetwork = connectionCountForUserInNetwork;
exports.findAllSocketsByUsernameInNetwork = findAllSocketsByUsernameInNetwork;
...
// ── Rooms / users ─────────────────────────────────────────────
function joinRoom(socketId, username, room, networkId) {
...
function createChannelInNetwork(networkId, name) {
    const clean = name.trim().toLowerCase().replace(/[^a-z0-9-_]/g, "-").slice(0, 40);
    if (!clean || clean.startsWith("dm:"))
        return false;
    ...
    channelSet.add(key);
    if (persistMode)
        (0, database_js_1.ensureRoom)(key);
    return isNew;
}
function listChannelsInNetwork(networkId) {
...
function deleteChannelInNetwork(networkId, name) {
    if (PROTECTED.has(name))
        return false;
...
function getAllUsersInNetwork(networkId) {
    const seen = new Map();
    for (const u of userMap.values()) {
...
function appendLogInNetwork(networkId, text) {
...
function getLogRoomInNetwork(networkId) {
    return `${networkId}:log`;
}
function searchMessagesInNetwork(networkId, query) {
    const prefix = `${networkId}:`;
    const q = query.toLowerCase();
    const results = [];
    for (const [room, msgs] of roomMessages) {
...
function connectionCountForUserInNetwork(username, networkId) {
    let count = 0;
    for (const u of userMap.values()) {
        if (u.username === username && u.networkId === networkId)
            count++;
    }
    return count;
}
function findAllSocketsByUsernameInNetwork(username, networkId) {
    const sockets = usernameToSockets.get(username);
    if (!sockets)
        return [];
