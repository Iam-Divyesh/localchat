    ...
    socket.on("file:chunk", (data) => {
        ...
        if (record.receivedChunks === record.totalChunks) {
            ...
            if (record.room.startsWith("dm:")) {
                ...
                const logMsg = (0, rooms_js_1.appendLogInNetwork)(networkId, `${record.uploadedBy} shared a file in DM (${record.name} · ${(record.size / 1024).toFixed(1)} KB)`);
                io.to(netRoomKey).emit("log:message", logMsg);
            }
            else {
                const serverRoom = `${networkId}:${record.room}`;
                io.to(serverRoom).emit("file:available", meta);
                const logMsg = (0, rooms_js_1.appendLogInNetwork)(networkId, `${record.uploadedBy} shared ${record.name} (${(record.size / 1024).toFixed(1)} KB) in #${record.room}`);
                io.to(netRoomKey).emit("log:message", logMsg);
            }
        }
    });
