...
router.post("/register", (req, res) => {
    ...
    try {
        const user = (0, database_js_1.registerUser)(email, username.trim(), password);
        const token = (0, database_js_1.createSession)(user.id);
...
router.post("/login", (req, res) => {
    ...
    const user = (0, database_js_1.loginUser)(email, password);
    if (!user)
...
router.post("/logout", (req, res) => {
    ...
    if (token)
        (0, database_js_1.deleteSession)(token);
...
router.patch("/username", (req, res) => {
    ...
    if (!username || username.trim().length < 2)
        return res.status(400).json({ error: "Username must be at least 2 characters" });
    const result = (0, database_js_1.updateUsername)(user.id, username.trim());
    if (!result.ok)
...
router.patch("/password", (req, res) => {
    ...
    const result = (0, database_js_1.updatePassword)(user.id, currentPassword, newPassword);
    if (!result.ok)
...
router.post("/reset-password", (req, res) => {
    ...
    if (newPassword.length < 6)
        return res.status(400).json({ error: "New password must be at least 6 characters" });
    const result = (0, database_js_1.resetPasswordByEmail)(email, currentPassword, newPassword);
    if (!result.ok)
        return res.status(400).json({ error: result.error });
    return res.json({ ok: true });
});
